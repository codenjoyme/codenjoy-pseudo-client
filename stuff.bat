...
:run   
    rem run jar
    rem call run :eval_echo ‘%JAVA% -Dfile.encoding=UTF-8 -jar %ROOT%\client-exec.jar “%GAME_TO_RUN%“ “%SERVER_URL%“ “%PSEUDO_RULES%“‘

    rem build & run (without jar)
    call run :eval_echo ‘%MVNW% clean compile exec:java -Dfile.encoding=UTF-8 -Dexec.mainClass=com.codenjoy.dojo.PseudoRunner -Dexec.args=“%GAME_TO_RUN% %SERVER_URL% %PSEUDO_RULES%“‘
    goto :eof